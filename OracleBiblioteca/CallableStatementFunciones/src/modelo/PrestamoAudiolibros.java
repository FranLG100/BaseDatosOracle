...
public class PrestamoAudiolibros {
	...
	public int insertarPrestamo(int idUsuario, int idLibro, String fechInicio) throws SQLException {
		...
		cstmt = con.prepareCall("{call INSERTAR_PRESTAMO_AUDIOLIBRO(?,?,?,?)}");
	...
	public void devolverPrestamo(int idPrestamo, String fechFin) throws SQLException {
		//EJEMPLO PROCEDIMIENTO
		con=Conexion.getConnection();
		cstmt = con.prepareCall("{call DEVOLVER_PRESTAMO_AUDIOLIBRO(?,?)}");
	...
	public void borrarPrestamo(int id) throws SQLException {
		//EJEMPLO PROCEDIMIENTO
		con=Conexion.getConnection();
		cstmt = con.prepareCall("{call BORRAR_PRESTAMO_AUDIOLIBRO(?)}");
		 cstmt.setInt(1, id);
         cstmt.execute();
         cstmt.close();
		con.close();
	...
	public DefaultTableModel listarPrestamo() {
		...
		try {
			con=Conexion.getConnection();
			cstmt=con.prepareCall("{call CONSULTA_PRESTAMOS_AUDIOLIBROS.obtener_prestamos_audiolibros(?)}");
